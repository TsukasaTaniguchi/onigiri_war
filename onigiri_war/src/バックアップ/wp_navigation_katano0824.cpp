...
MyPose way_point[] = {
    { 1.3,  0.0, 0.0},
    { 1.3,  0.0, M_PI/4},
    { 1.3 + 2.00 * sin(M_PI/4), 2.00 * cos(M_PI/4), M_PI/4},
    { 1.3 + 2.00 * sin(M_PI/4), 2.00 * cos(M_PI/4), -M_PI/4},
    { 1.3 + 2.00 * sin(M_PI/4) + 1.2 * sin(M_PI/4), 2.00 * cos(M_PI/4) - 1.2 * cos(M_PI/4), -M_PI/4},
...
static const std::string LASER_WINDOW = "Image window";
...
void scancallback(const sensor_msgs::LaserScan::ConstPtr& input) {
	...
	for (int i = 0; i < input->ranges.size(); ++i) {
		...
		pc[i][0]=(double(range)*cos(i/180+yaw2)+odomx)*cos(-M_PI/4)-(double(range)*sin(i/180+yaw2)+odomy)*sin(-M_PI);
		pc[i][1]=(double(range)*sin(i/180+yaw2)+odomx)*sin(-M_PI/4)+(double(range)*sin(i/180+yaw2)+odomy)*cos(-M_PI);
	}
}
...
*/
		for(int i=0;i<701;i++){
		    for(int j=0;j<701;j++){
		      for(int k=0;k<359;k++){
			if(i*0.005<=pc[k][0]<(i+1)*0.005 && j*0.005<=pc[k][1]<(j+1)*0.005){
