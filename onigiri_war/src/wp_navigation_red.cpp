...
MyPose way_point[] = {
    ...
    { 2.00 , 1.04 * cos(M_PI/4)-0.1, -M_PI/2},
    { 2.00 , 1.04 * cos(M_PI/4), -M_PI/2},
    { 2.00 , 1.04 * cos(M_PI/4), 0.0},
    { 2.50 , 1.04 * cos(M_PI/4), 0.0},
    { 2.00 + (2.00-1.25 + 1.04 * sin(M_PI/4)), 1.04 * cos(M_PI/4), 0},
...
*/
	void sendWaypoint_laser(int n_ptr){
		...
		if(((phi-yaw2)>0.5*M_PI/180 || (phi-yaw2)<-0.5*M_PI/180) && (t_tar>0.01 || t_tar<-0.01)){//step1
			m_frontspeed=0;
		...
		else if(((phi-yaw2)<0.5*M_PI/180&&(phi-yaw2)>-0.5*M_PI/180) && (t_tar>0.01 || t_tar<-0.01)){//step2
		...
		else if(((way_point[n_ptr].yaw-yaw2)>0.5*M_PI/180||(way_point[n_ptr].yaw-yaw2)<-0.5*M_PI/180) && (t_tar<0.01 && t_tar>-0.01) ){//step3
			m_frontspeed=0;
		...
		else if(((way_point[n_ptr].yaw-yaw2)<0.5*M_PI/180&&(way_point[n_ptr].yaw-yaw2)>-0.5*M_PI/180) && (t_tar<0.01 && t_tar>-0.01)){//step4
			m_frontspeed=0;
			m_turnspeed=0;
			m_destPnt++;	
			ROS_INFO("step4!!");
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		robo_ctrl.moveRobo();
		//購読するtopic名
		odom_sub_loop = n.subscribe("/red_bot/odom", 1000, odomCallback2);
		scan_sub = n.subscribe<sensor_msgs::LaserScan>("/red_bot/scan", 1000, scancallback);
                //ROS_INFO("odom x y yaw= %f %f %f", odomx,odomy,yaw2);
